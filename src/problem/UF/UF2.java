    ...
    public void calculatorObject(Individual individual) {
        ...
        for (int j = 2; j <= numberOfVariables; j++) {
            if (j % 2 == 0) {
                /*
                 * yj = x[j-1]-0.3*x[0]*
                 * (x[0]*Math.cos(24.0*Math.PI*x[0]+4.0*j*Math
                 * .PI/numberOfVariables_)+2.0)*
                ...
                yj = x[j - 1]
                        ...
                        * x[0]
                        * x[0]
                        * Math.cos(24 * Math.PI * x[0] + 4 * j
                        * Math.PI / numberOfVariables) + 0.6 * x[0])
                ...
                yj = x[j - 1]
                        ...
                        * Math.cos(24 * Math.PI * x[0] + 4 * j
                        * Math.PI / numberOfVariables) + 0.6 * x[0])
                        * Math.cos(6.0 * Math.PI * x[0] + j * Math.PI
                        / numberOfVariables);
        ...
        functionVector.add(1.0 - Math.sqrt(x[0]) + 2.0 * sum2 / (double) count2);
