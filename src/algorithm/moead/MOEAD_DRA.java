...
/**
 * MOEAD_DRA:与MOEAD_original相比：增加了匹配约束、差分和动态资源分配。
 * 
 * @author 张作峰
 * 
 */
public class MOEAD_DRA extends Algorithm {
	 ...
	 * 用于计算效用函数值是储存旧值的数组
	 */
	private ArrayList<ArrayList<Double>> oldValues;
	/**
	 * 效用函数值
	 */
	 ...
	 * 更新频率
	 */
	private double[] freques;

	public MOEAD_DRA() {
		algorithmName = "MOEAD_DRA";
	}

	public MOEAD_DRA(String name) {
		super();
	}
	...
	public List<Integer> tour_selection(int depth) {
		...
		for (int k = 0; k < problem.getObjectNum(); k++)
			selected.add(k);
		for (int n = problem.getObjectNum(); n < populationSize; n++)
			candidate.add(n);

		while (selected.size() < (int) (populationSize / 5.0)) {
			int best_idd = (int) (Math.random() * candidate.size());
			int i2;
			int best_sub = candidate.get(best_idd);
			int s2;
			for (int i = 1; i < depth; i++) {
				i2 = (int) (Math.random() * candidate.size());
				s2 = candidate.get(i2);
				if (utils[s2] > utils[best_sub]) {
					best_idd = i2;
					best_sub = s2;
				}
			}
	...
	public void comp_utility() {
		...
		for (int n = 0; n < populationSize; n++) {
			ArrayList<Double> functionVector = populationVector.get(n)
					.getFunctionVector();
			f1 = decomposition.comput(objectNum, functionVector,
					populationVector.get(n).getWeightVector(),
					idealIndividuals, referenceVector);
			f2 = decomposition.comput(objectNum, oldValues.get(n),
					populationVector.get(n).getWeightVector(),
					idealIndividuals, referenceVector);
			...
			if (delta > 0.001)
				...
				uti = 0.95 * (1.0 + delta / 0.001) * utils[n];
				utils[n] = uti < 1.0 ? uti : 1.0;
			}
			// 更新oldValue
			ArrayList<Double> arrayList = new ArrayList<Double>();
			for (int j = 0; j < objectNum; j++) {
				arrayList.add(functionVector.get(j));
			}
			oldValues.set(n, arrayList);
		}
	}
	...
	public void init() {
		...
		if (problem.getObjectNum() == 2) {
			...
			for (int i = 0; i < H; i++) {
				...
				weightVector.add((double) (H - 1 - i) / H);
		...
		oldValues = new ArrayList<ArrayList<Double>>();
		utils = new double[populationSize];
		freques = new double[populationSize];
		...
		for (int i = 0; i < populationSize; i++) {
			...
			if (problem != null) {// 个体编码的初始化
				...
				ArrayList<Double> arrayList = new ArrayList<Double>();
				for (int j = 0; j < objectNum; j++) {
					arrayList.add(individual.getFunctionVector().get(j));
				}
				oldValues.add(arrayList);
				utils[i] = 1;
				freques[i] = 0;
			} else {
